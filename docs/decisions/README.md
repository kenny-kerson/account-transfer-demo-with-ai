...
| [0010](0010-idempotency-concurrency.md) | idempotency-concurrency | **멱등성·동시성 제어: 서버 GUID 2-Phase + 다층 락 + 핑거프린트 관찰** | 2026-05-25 | Accepted *(Amended by 0014)* | Data Model |
...
| [0014](0014-idempotency-key-extraction.md) | idempotency-key-extraction | **IdempotencyKey 별도 테이블 분리: Transfer에서 멱등 컬럼(phase·캐시) 추출 → D-12로 위임 (Stripe 류 산업 표준)** | 2026-05-25 | Accepted | Data Model |

## 참고 지식 (Knowledge Base)

> 의사결정의 근거가 된 외부 이론·패턴 + **결정 과정에서 진행된 다단계 분석·시나리오 검토·트레이드오프 논의**를 향후 학습·재검토 가능하도록 보존한다.

- [매핑 전략 — 톰 홈버그 4가지 전략](../references/mapping-strategies.md) — ADR-0013 근거
- [Idempotency 설계 검토](../references/idempotency-design-discussion.md) — ADR-0010·0014 배경 분석 (transferStatus vs idempotencyPhase 비교, 시나리오 표, 산업 표준 비교 보존)
